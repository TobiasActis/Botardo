"""
Script para extraer datos de la Base Monetaria Total desde FRED
...
try:
    ...
    # Guardar el JSON
    archivo_salida = "BaseMonetariaTotal.json"
    with open(archivo_salida, 'w', encoding='utf-8') as f:
        json.dump(datos_json, f, indent=2, ensure_ascii=False)
    
    print(f"\n[OK] Archivo JSON creado exitosamente: {archivo_salida}")
    print(f"\nEstadisticas:")
    print(f"  Valor minimo: ${df['valor'].min():,.2f} millones")
    print(f"  Valor maximo: ${df['valor'].max():,.2f} millones")
    print(f"  Valor promedio: ${df['valor'].mean():,.2f} millones")
    print(f"  Ultimo valor: ${df['valor'].iloc[-1]:,.2f} millones ({df['fecha'].iloc[-1].strftime('%Y-%m-%d')})")
    ...
    try:
        ...
        print("https://fred.stlouisfed.org/series/BOGMBASE")
